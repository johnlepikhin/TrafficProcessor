...
ChunkIPv6 *ParserIPv6::Process(Data *data, Chunk *p)
{
	...
	if (parentDIX) {
		...
		if (parentEthernet && parentEthernet->EtherNetType == 0x86dd) {
			IPv6Addr *srcIP = new IPv6Addr(data, 8);
			IPv6Addr *dstIP = new IPv6Addr(data, 24);

			unsigned char nextHdr = (unsigned char)data->getPtrAtOffset(2)[0];
			data->ignore(40);
			while (0 == nextHdr
					|| 43 == nextHdr
					|| 44 == nextHdr
					...
					|| 51 == nextHdr
					|| 60 == nextHdr) {
				nextHdr = (unsigned char)data->getPtrAtOffset(0)[0];
				data->ignore(4);
			}
