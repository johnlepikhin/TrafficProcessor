...
#include "../../src/levels/ParserEtherNetDIX.h"
#include "../../src/levels/ParserIPv4.h"
#include "../../src/types/IPv4Addr.h"

namespace testParserIPv4 {
	...
	TEST(levels_ParserIPv4, HandlesUnderFlowData) {
		struct Test {
			static void test () {
				std::stringstream input;
	...
	TEST(levels_ParserIPv4, HandlesDoParseCheckContent) {
		...
		EXPECT_STREQ("192.168.57.2", chunk->DstIP->asString().c_str());
		EXPECT_EQ(0x62, chunk->PktLength);
